...
# Load modules
module purge
module load gcc cuda cray-mpich
ml cudnn/8.8.1.3-12
module load conda
conda activate /glade/derecho/scratch/benkirk/derecho-pytorch-mpi/env-pytorch-v2.3.1-torchvision-v0.18-derecho-gcc-12.2.0-cray-mpich-8.1.27
...
echo "------------------------------------" >> $LOGFILE
CMD="MASTER_ADDR=$head_node_ip MASTER_PORT=1234 mpiexec -np 8 --cpu-bind none ./main.py --backend nccl"
echo $CMD | tee -a $LOGFILE
eval $CMD


echo "------------------------------------" >> $LOGFILE
CMD="MASTER_ADDR=$head_node_ip MASTER_PORT=1234 mpiexec -np 8 --cpu-bind none ./main.py --backend mpi"
echo $CMD | tee -a $LOGFILE
eval $CMD


echo "------------------------------------" >> $LOGFILE
CMD="MASTER_ADDR=$head_node_ip MASTER_PORT=1234 mpiexec -np 8 --cpu-bind none ./main.py --backend gloo"
echo $CMD | tee -a $LOGFILE
eval $CMD
